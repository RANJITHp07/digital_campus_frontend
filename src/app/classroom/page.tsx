...
import { DUE_DATES } from "@/apis/assignment/query";
...
const Classroom = () => {
  ...
  const {
    ...
    type,
    category,
  } = useAppSelector((state) => state.classroomReducer);
      ...
      <div className="xm:flex">
        ...
        <div className={`${type !== "calendar" && "w-full"}`}>
          {type === "home" && !categoryType && (
            <div
              className={` flex-wrap flex lg:ml-7 xl:ml-12 md:ml-12 ${
                !state && "xm:ml-0"
              }`}
            >
              {loading ? (
                ...
                <Suspense fallback={loading}>
                  {filteredClassroom &&
                  filteredClassroom.getFilteredClassroom.length > 0 ? (
                    filteredClassroom.getFilteredClassroom.map(
                      (c: ClassroomProps) => (
                        <div
                          className={` mx-auto md:mx-0 ${
                            state ? "xl:w-1/3 xm:w-1/2" : "xl:w-1/4 xm:w-1/3"
                          } lg:w-1/3`}
                          key={c._id}
                        >
                          <Class
                            ...
                            type={
                              !c.students_enrolled ||
                              !c.students_enrolled.includes(
                                token.id?.toString() as string
                              )
                            }
          ...
          {type === "home" && categoryType && (
            ...
            >
              {loading ? (
                ...
                <Suspense fallback={loading}>
                  {allClassroom &&
                  allClassroom.getAllTheClassroom.length > 0 ? (
                    allClassroom.getAllTheClassroom
                      .slice(pagination * 9 - 9, pagination * 9)
                      .map((c: ClassroomProps) => (
                        <div
                          className={` mx-auto md:mx-0 ${
                            state ? "xl:w-1/3 xm:w-1/2" : "xl:w-1/4 xm:w-1/3"
                          } lg:w-1/3`}
          ...
          {type === "teaching" && (
            ...
            >
              {loading ? (
                ...
                <Suspense fallback={loading}>
                  {data && data.getCreatorClassroom.length > 0 ? (
                    data.getCreatorClassroom
                      .slice(pagination * 9 - 9, pagination * 9)
                      .map((c: ClassroomProps) => (
                        <div
                          className={` mx-auto md:mx-0 ${
          ...
          {type === "enrolled" && (
            ...
            >
              {loading ? (
                ...
                <Suspense fallback={loading}>
                  {addedClassroom &&
                  addedClassroom.getAllClassroom.length > 0 ? (
                    addedClassroom.getAllClassroom
                      .slice(pagination * 9 - 9, pagination * 9)
                      .map((c: any) => (
                        <div
                          className={` mx-auto md:mx-0 ${
          ...
          {type === "profile" && (
            <div className="w-full justify-center">
              <Profile
                addedClassroom={addedClasroom}
                createdClassroom={createdClasroom}
              />
            </div>
          )}
          {type !== "calendar" &&
            ((data && type === "teaching" && data.getCreatorClassroom.length) ||
              (filteredClassroom &&
                filteredClassroom.getFilteredClassroom.length > 0) ||
              (addedClassroom &&
                type === "enrolled" &&
