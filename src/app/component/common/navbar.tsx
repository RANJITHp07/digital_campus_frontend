...
import { ADD_REQUEST, CREATE_CLASS } from "@/apis/classroom/mutation";
  ...
  const handleChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    try {
      if (e.target.value.length !== 0 && filter.length > 0) {
        ...
        const newFilter = data.getAllTheClassroom.filter(
          (value: ClassroomProps) => {
    ...
    <nav className="flex justify-between items-center w-full bg-white">
      <div className="p-3 flex items-center">
        <div className="hidden xm:block">
          <MenuIcon
            className="mt-2 mr-3 text-[#3b6a87] cursor-pointer"
            onClick={() => dispatched(changeModalState())}
          />
        </div>
        <div className="xm:hidden">
          <MenuIcon
            className="mt-2 mr-3 text-[#3b6a87] cursor-pointer  lg:hidden"
            onClick={() => dispatch({ type: "SET_OPEN", value: !open })}
          />
        </div>
        <Link href={"/classroom"}>
          <Image
      ...
      <div className="flex   mx-3 ">
        <div className=" hidden md:flex items-center rounded-full mx-3 border-2 border-slate-300 px-3 py-1">
          ...
          <input
            ...
            className="text-[#8da3b1] border-slate-300 focus:outline-none  text  w-96 "
          />
        </div>
        ...
        {state && (
          <div className="absolute top-16 mx-5 w-[26rem] bg-white box_shadow p-2 rounded-lg z-50">
            {filter.length > 0 ? (
              filter.map((m: ClassroomProps) => {
                return (
                  <div key={m._id}>
                    <p
                      className="text-slate-500 text my-3  cursor-pointer"
                      onClick={() => navigation.push(`/classroom/${m._id}`)}
                    >
                      {m.className}
                    </p>
                    <hr />
              ...
              <p className="text-slate-300 text cursor-pointer">
                No such classroom
              </p>
            )}
          </div>
      ...
      {open && (
        <div className="fixed top-[4.4rem] left-0 right-0 bottom-0 flex  bg-black bg-opacity-50 z-50">
          <hr />
          <SidePanel />
        </div>
      )}
      <Suspense fallback={<div>Loading...</div>}>
        <Modal
          ...
          open={open1}
          footer={null}
          onCancel={() => dispatch({ type: "SET_OPEN1", value: false })}
        >
          <form className="mt-5" onSubmit={Onsubmit}>
            ...
            />
            <select
              className=" w-full  p-2 rounded-md focus:outline-none border-slate-300 text-slate-500   border-2 my-2 text "
            ...
            <div className="flex justify-end my-2">
              <button
                type="submit"
                className="p-2 border-2 bg-[#3b6a87] text-white rounded-md px-4 text "
              >
                Create
              </button>
            </div>
        ...
        <Modal
          ...
          open={open2}
          footer={null}
          onCancel={() => dispatch({ type: "SET_OPEN2", value: false })}
        >
          <input
            type="text"
            className=" w-full p-2 rounded-md focus:outline-none border-slate-300 text-slate-500  border-2 my-2 text"
          ...
          <div className="flex justify-end my-2">
            <button
              type="submit"
              className="bg-[#3b6a87] p-2 border-2 text-white rounded-md px-4 text "
              onClick={() => handleJoin()}
            >
              Join
            </button>
