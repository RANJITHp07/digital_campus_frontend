...
import { FETCH_CLASSROOM_DETAILS, UPDATE_CLASS } from '@/apis/classroom';
...
const [createAssignment]=useMutation(CREATE_ASSIGNMENT,{
  client:assignmentClient,
  refetchQueries: [{ query: FETCH_ASSIGNMENT_DETAILS ,variables:{id:id}}],
  onError(err){
    console.log(err)
...
const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
  ...
  function handleCopyClick (){
    navigator.clipboard.writeText(data.getClassroomDetails.classCode)
      .then(() => {
  ...
  return (
    <div className='xl:mx-24 lg:mx-24 xm:mx-12 md:mx-6 mx-5 h-min-screen'>
     { loading ? <LoadinPage/> :
       <>
        <div 
        className={`bg-cover  bg-right h-100 mt-9 w-full h-60 box_shadow  ${!detail ? "rounded-lg my-9" : 'rounded-t-lg'} relative`}
    style={{
      backgroundImage:  `url( ${background})`,
    }}
    >
      <div className="flex justify-end">
        {assign && <button className='bg-white box_shadow p-2 px-3 text-center m-3 rounded-md text-sm text text-[#3b6a127]' onClick={()=>setopen(true)}><ModeEditOutlinedIcon className='text-md text-[#3b6a127]'/> Customize</button>}
        </div>
        ...
        {
          detail &&
          <div className={`bg-white box_shadow  rounded-b-lg mb-9 p-5 text`}
          style={{ color: text[1] }}>
           <p > Name of the classroom : {data.getClassroomDetails.className} </p>
           <p> Classcode : {data.getClassroomDetails.classCode} <ContentCopyOutlinedIcon className='text-xs cursor-pointer'/></p>
           <p>No of students enrolled : {data.getClassroomDetails.students_enrolled.length} <GroupsIcon className='text-sm'/></p>
          </div>

        }
        <div className='md:flex my-5'>
          <div className='box_shadow   rounded-md w-36 h-24 hidden md:block'>
            <div className='flex justify-between w-full' style={{ color: text[1] }}>
          ...
          <div className='w-full md:ml-6'>
            <div className='box_shadow p-4 rounded-md h-24'>
              <p className={` text-xs mx-1`} style={{ color: text[1] }}>Announcment for all the participants in classroom</p>
              <input
              id="message"
              type="text"
              placeholder="Message.."
            ...
            />
              ...
              <div className='w-24 flex justify-center items-center'>
                   ...
                   style={{ backgroundColor: text[1] }}>
                    {
                      m.assignmentType==="Announcement" &&  <NewReleasesIcon className='text-white text-xl'/>
                    }
                   {
                    m.assignmentType==="Question" && <LiveHelpOutlinedIcon className='text-white text-xl'/>
                   }
                     {
                      ...
                      {
                       m.assignmentType==="Material" && <MenuBookOutlinedIcon className='text-white text-xl'/>
                     }
                     {
                      ...
                      <div>
                        {
                          m.assignmentType==="Announcement"? <><p className={`text ${text[0]}`}>Announcment from {data.getClassroomDetails.creator} :
                          </p>
                          <p className='text text-slate-500'>{m.title}</p>
                          </> :  <p className={`text ${text[0]}`}>{data.getClassroomDetails.creator} posted a {m.assignmentType}</p>
                        }
        ...
        <Modal title={<span className='text font-normal text-[#3b6a87]'>Customize apperance</span>}  open={open} footer={null} onCancel={()=>setopen(false)} width={1000}>
    ...
    {Object.keys(color).map((c) => (
          <div
            key={c}
            className={`w-16 h-16 md:w-24 md:h-24 rounded-full text-white flex justify-center items-center cursor-pointer ${c === '#ef4444' || c === '#10b981' ? "hidden md:flex" : ""}`}
style={{ backgroundColor: c }}
    ...
    <button className='bg-slate-300 px-4 py-2 text rounded-md mx-3 ' onClick={handleSave}>Save</button>
    </div>
        </Modal>
        </>
       }
