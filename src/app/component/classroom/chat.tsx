    ...
    <div className="bg-white h-full">
      <div className="bg-[#3b6a87] text-white p-2 flex justify-between items-center ">
        ...
        <CloseIcon
          ...
          onClick={() => dispatch(changeChatState(false))}
        />
      </div>
      <div className="h-[87%] xl:h-[84%] lg:h-[85%] p-4 overflow-auto relative">
        {isHovered && (
          <div
            className="fixed bottom-8 md:bottom-16 xm:bottom-9 lg:bottom-12 right-11 z-50"
            onMouseLeave={() => {
              setTimeout(() => {
                setIshovered(false);
              }, 800);
          ...
          >
            <div
              className="box_shadow p-2 rounded-full text-[#4b606d] delay-00 bg-white hover:scale-105 hover:bg-slate-100"
              onClick={() => {
            ...
            <div
              className="box_shadow p-2 rounded-full my-3 text-[#4b606d]  bg-white hover:scale-105 hover:bg-slate-100"
              onClick={() => {
              ...
              >
                <div>
                  <div className="flex items-end ">
                    <Image
                      src={
                        m.sender.user_id
                          ? m.sender.user_id === token.id
                            ? "/profile-logo.jpg"
                            : m.sender.profile.length === 0
                            ? "/profile-logo.jpg"
                            : m.sender.profile.length
                          : "/profile-logo.jpg"
                      }
                      width={20}
                      height={20}
                    ...
                    />
                    <p className="text-xs text mx-1 text-slate-500">
                      {m.sender.user_id
                        ? m.sender.user_id === token.id
                          ? token.name
                          : m.sender.username
                  ...
                  {m.text.type === "photo" && (
                    ...
                    >
                      <Image
                        ...
                        height={300}
                        alt="profile"
                        className="text-center w-[19rem] cursor-pointer  object-contain"
                      />
                    </a>
                  )}
                  {m.text.type === "vedio" && (
                    <a
                      ...
                      rel="noopener noreferrer"
                    >
                      <video src={m.text.text} controls className="w-[19rem]" />
                    </a>
                  )}
                  {m.text.type === "pdf" && (
                    <a
                  ...
                  {m.text.desc && (
                    <p className="bg-slate-100 text w-[19rem] text-[#4b606d] rounded-b-md text-sm p-3">
                      {m.text.desc}
                    </p>
                  )}
      ...
      <div className=" h-[8%] flex items-center justify-between bg-slate-100">
        ...
        <EmojiEmotionsIcon
          className="mx-3 text-[#4b606d] cursor-pointer"
          onClick={() => setemojii(!emojii)}
        />
        <input
          type="text"
          className="w-full h-full focus:outline-none bg-slate-100 text"
        ...
        />
        <AttachFileIcon
          className="rotate-45 mx-3 text-[#4b606d] cursor-pointer my-1"
          id="file"
      ...
      <Modal
        title={<span className="text font-normal text-slate-500">Post</span>}
      ...
      >
        <div className="flex justify-center">
          ...
          {type === "photo" && file && (
            <Image
              ...
              height={300}
              alt="profile"
              className="text-center object-cover w-56 h-56"
            />
          )}
          {type === "vedio" && file && (
            <video
        ...
        <div className="flex bg-slate-200 w-full p-2 mt-5 rounded-full">
          <input
            type="text"
            placeholder="Add message with the post"
            className="text focus:outline-none w-full bg-slate-200"
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              setdesc(e.target.value)
            }
          />
          <SendIcon
            className="text-[#4b606d] cursor-pointer"
