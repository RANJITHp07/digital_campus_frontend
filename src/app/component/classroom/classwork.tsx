...
import { CREATE_ASSIGNMENT, FETCH_MAINTOPIC, GROUPED_ASSIGNMENT } from '@/apis/assignment';
  ...
  const items: MenuProps['items'] = [
    {
      key: '1',
      label: (
        <a  href={`/classroom/createAssignment?classroom=${id}&type=Assignment`} ><span className='text text-[#3b6a87] my-2'><AssignmentIcon className='text-lg'/> Assignment</span></a> 
      ),
    },
    {
        key: '2',
        label: (
          <p className='text text-[#3b6a87] my-2'><HelpCenterIcon className='text-lg'/> Question</p> 
        ),
      },
    {
      key: '3',
      label: (
        ...
        key: '4',
        label: (
          <a href={`/classroom/createAssignment?classroom=${id}&type=Material`} className='text text-[#3b6a87] my-2'><span className='text text-[#3b6a87] my-2'><AutoStoriesIcon className='text-lg'/> Material</span></a>
        ),
      },
      {
        key: '5',
        label: (
          <p className='text text-[#3b6a87] my-2'  onClick={()=>setopen(true)}><PollIcon className='text-lg'/> Polling</p>
        ),
      }
    ...
    <div className='flex justify-center'>
        <div className=' w-full md:w-3/4 my-9'>
           ...
           <div className='border-2 border-[#7195aa]  w-12/12 mx-2 md:w-1/2 xl:w-1/3 h-12 flex items-center justify-between'>
             ...
             <ArrowDropDownIcon className='text-[#3b6a87] cursor-pointer' onClick={()=>{
              setassignment(data.getgroupedAssignment)
               setstate(true)
             }}/>
           </div>
           {
            state &&  <div className='bg-white absolute box_shadow mx-2  w-[20rem] p-2 rounded-md'>
                 ...
                 <hr/>
            {
             mainTopic && mainTopic.getdistinctmainTopic.mainTopic.map((m:string)=>{
               return (
                 <>
                 <p className='my-3 cursor-pointer text text-slate-500' onClick={()=>handleTopic(m)}>{m}</p>
                 <hr/>
                 </>
            ...
            {
              assignment.length>0 ?assignment.map((a:any)=>{
                return (
                  <div className="my-9">
                  <div className='flex justify-between items-center'>
                <p className='text-3xl text text-[#3b6a87] mx-4'>{a._id}</p>
                </div>
                {
                  a._id &&  <hr className='border-1 rounded-full border-[#3b6a87] mb-6 mt-3'/>
                }
           
           {
            ...
            <hr/>
              ...
              }) :  <Image src={'/noassignment.jpg'} width={300} height={300} alt='photo' className='mx-auto'/>
            } 
            </>
      }
        </div>
        <Modal title='Polling for the students' open={open} footer={null} onCancel={()=>
         ...
         <input type="text" className=" w-11/12 p-2 focus:outline-none  text" placeholder='Ask a polling question' value={question.question1} onChange={(e:ChangeEvent<HTMLInputElement>)=>
         {
          setquestion({
        ...
        <AddIcon className='m-1 text-slate-400 cursor-pointer' onClick={()=>
        {
          setadd(true)
