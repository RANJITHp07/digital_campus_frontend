...
import {  DELETE_ASSIGNMENT, EDIT_ASSIGNMENT, EDIT_POLLING } from '@/apis/assignment';
...
function Material({material,assignment,setassignment,id}:{material:any,assignment:any,setassignment:React.Dispatch<any>,id:string}) {
  const items: MenuProps['items']=[
    ...
    const updatePolling=async()=>{
       ...
       const { __typename: type,students, title,polling: { __typename, ...p } } = polling;
       const a = { title,students, polling: { ...p } };
       const assignment={...a,creator:token.name,class_id:[id],assignmentType:"Polling"}
   ...
   // to open the model to delete
    ...
    const handleAddoptions=()=>{
      setPolling({...polling,polling:{answers:[option,...polling.polling.answers]}})
  ...
  return (
    <div>
        <div className='my-4 flex justify-between items-center'>
            ...
            <Modal title={<span className='text font-normal text-[#3b6a87]'>Edit Polling</span>} open={open} footer={null} onCancel={()=>setopen(false)}>
              <div className='border-2 rounded-md flex items-center'>
              <input type='text' defaultValue={polling.title[0].toUpperCase() + polling.title.slice(1,polling.title.length).toLocaleLowerCase()} className='p-2 focus:outline-none rounded-md w-full'
              onChange={(e:ChangeEvent<HTMLInputElement>)=>setPolling({...polling,title:e.target.value})} />
            <AddIcon className='m-1 text-slate-400 cursor-pointer'
            onClick={()=>{
          ...
          <input type='text' placeholder={`Option`} className=' w-full text focus:outline-none' 
           onChange={(e:ChangeEvent<HTMLInputElement>)=>setOption(e.target.value)} />
          <button className='text text-[#3b6a87]  p-1' onClick={handleAddoptions}>Add</button>
        </div>
              }
              <div className='mt-5'>
                {
                 !loading ? polling.polling.answers.map((m,index)=>{
                    return (
                      <>
                      ...
                      {
                        editIndex!==index ? 
                        <p className='text w-10/12'>{m[0].toUpperCase() + m.slice(1,m.length).toLocaleLowerCase()}</p>
                        :
                        <input type='text' className='focus:outline-none' defaultValue={m[0].toUpperCase() + m.slice(1,polling.title.length).toLocaleLowerCase()} 
                         onChange={(e:ChangeEvent<HTMLInputElement>)=>setText(e.target.value)}  autoFocus />
                      }
                      </div>
                      {
                        editIndex!==index ? 
                        <>
                         <ModeEditOutlineOutlinedIcon className='m-1 text-slate-400 cursor-pointer' onClick={()=>
                        {
                          setEditIndex(index)
                        }
                        }/>
                      <ClearIcon className='m-1 text-slate-400 cursor-pointer' onClick={()=>handleRemove(index)}/>
                        </>
                        :
                        <DoneOutlined className='m-1 text-slate-400 cursor-pointer' onClick={()=>handlEditOptions(index)}/>
                      }
                  ...
                  :
                  <div className='flex justify-center'><CircularProgress className='text-[#3b6a87]'/></div>
                }
              <div className="flex justify-end mt-3">
              <button type='submit' className="text-white p-2 border-2 bg-[#3b6a87] rounded-md px-4 text " onClick={updatePolling}>Update</button>
          </div>
              </div>
            </Modal>
    </div>
