    ...
    <div className="mx-6 my-9 md:my-7 xm:my-4  lg:my-7">
      <form onSubmit={handleSubmit((data) => (page ? handleSignup(data) : handleLogin(data)))}>
        ...
        <div className={` ${!page ? 'w-full' : 'md:w-1/2 md:mr-3'}`}>
          ...
          {text && page && <p className='text-xs text-red-500'>minimum 8 charcters must be there</p>}
        </div>
        {page && (
          <div className='md:w-1/2'>
            <label className='text-xs font-medium text-gray-400'>
              {!errors.confirm_password && 'confirm_password'}
              <span className='text-red-500'>{errors.confirm_password ? errors.confirm_password.message : '*'}</span>
            </label>
            <input
              id='password'
              type='password'
        ...
        <button
          type='submit'
          className={`bg-[#194866] ${page ? 'my-5' : 'mt-5'} w-full text-white p-3 rounded-md`}>
          {page ? (loading ? <CircularProgress className='text-white' /> : resend ? 'Resend Otp' : 'Signup') : loading ? (
